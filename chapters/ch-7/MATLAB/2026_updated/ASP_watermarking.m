...
audio_signal = audioread('..\..\audio_samples\violin.wav',[1 N_bits*N_samples]);
...
hold off;
xlabel('Normalized Frequency : f/(Fs/2)');
legend('Audio signal', 'Masking threshold', 'Filter response', 'Location', 'northeast');
...
for m = 0:fix(N_bits*N_samples/N_samples_PAM)-1
    ...
    % |equalized_signal|, for frame 10.
        ...
        hold off;
        xlabel('Normalized Frequency : f/(Fs/2)');
        legend('Equalized signal', 'Equalizer response', 'Location', 'northeast');
    end;
...
xlabel('Normalized Frequency : f/(Fs/2)');
legend('Audio signal', 'Masking threshold', 'Location', 'northeast');
axis([0 1 -100 0]);
