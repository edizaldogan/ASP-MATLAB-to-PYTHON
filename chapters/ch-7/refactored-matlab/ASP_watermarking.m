...
audio_signal = audioread('C:\Users\hp\Documents\Erasmus\Project\ASP_Project\chapters\ch-7\audio_samples\violin.wav', [1 N_bits*N_samples]);
...
hold off;
xlabel('Normalized Frequency : f/(Fs/2)');
legend('Audio signal', 'Masking threshold', 'Filter response');
...
for m = 0:fix(N_bits*N_samples/N_samples_PAM)-1
    ...
    % |equalized_signal|, for frame 10.
        ...
        hold off;
        xlabel('Normalized Frequency : f/(Fs/2)');
        legend('Equalized signal', 'Equalizer response');
    end;
...
xlabel('Normalized Frequency : f/(Fs/2)');
legend('Audio signal', 'Masking threshold');
axis([0 1 -100 0]);
