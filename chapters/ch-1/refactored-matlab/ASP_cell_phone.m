...
try
    ...
    %%
    % The file is about 1.1 s log (9000 samples). One can easily spot the
    ...
    for i=1:(length(speech)-frame_length+frame_shift)/frame_shift
        ...
        % the excitation
        [X, z_gamma_e] = filter(1, ai_perceptual, excitation, ...
            z_gamma_e);
        ...
        if i==135
            clf;
            [H,~]=freqz(1,ai, 512);
            [HS,~]=freqz(input_frame,1, 512);
            [HP,~]=freqz(ai, ai_perceptual, 512);
    ...
    fprintf('\n[ STATUS: SUCCESS ] Script completed without errors.\n');
...
function auto_save_fig()
    ...
    currentFolder = 'C:\Users\hp\Documents\Erasmus\Project\ASP_Project\chapters\ch-1\original-matlab';
