...
with st.sidebar:
    st.markdown("---")
    audio_bytes = audio_recorder(text="Or record your voice", icon_size="2x")

if uploaded_file is None and not audio_bytes:
...
if uploaded_file is not None or audio_bytes:

    lpc_order = st.sidebar.slider("LPC Order (p)", min_value=2, max_value=60, value=12, step=1)

    if audio_bytes:
        audio, fs = sf.read(io.BytesIO(audio_bytes))
    else:
    ...
    if not duration <= frame_duration:
        ...
        if np.max(np.abs(frame)) < 1e-8:
            frame = frame + np.random.randn(len(frame)) * 1e-5
        ...
        audio_for_playback = audio * 0.001   # 30% of original amplitude
        st.audio(audio_for_playback, sample_rate=fs)
