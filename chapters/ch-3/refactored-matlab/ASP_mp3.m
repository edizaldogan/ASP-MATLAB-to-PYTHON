...
% Order of the following 5 lines are important!
% onCleanup must be used after the configuration.
clc;
close all;
clear auto_save_fig;
set(0, 'DefaultFigureVisible', 'off'); 
security = onCleanup(@()Secure_Close);
...
try
    ...
    title('Spectrogram Old Version');
    ylabel('Frequency (Hz)');
    xlabel('Time (s)');
    auto_save_fig()
    
    % spectrogram(x,win,nOverlap,freqSpec,Fs)
    x = input_signal; win = hanning(256); nOverlap = 128;
    freqSpec = 1024;
    [S, F, T] = spectrogram(x,win,nOverlap,freqSpec,Fs);
    imagesc(T, F, 20*log10(abs(S)));
    axis xy;
    colormap(jet);
    title('Spectrogram New Version');
    ylabel('Frequency (Hz)');
    xlabel('Time (s)');
    auto_save_fig()
    ...
    auto_save_fig()
    soundsc(synt_signal(2001:end),Fs);
    ...
    clf;
    specgram(G0_output,1024,Fs,256);
    auto_save_fig()
    %soundsc(G0_output,Fs);
    ...
    clf;
    specgram(G1_output,1024,Fs,256);
    auto_save_fig()
    %soundsc(G1_output,Fs);
    ...
    clf;
    specgram(synt_signal,1024,Fs,256);
    auto_save_fig()
    %soundsc(synt_signal,Fs);
    ...
    [input_signal, Fs] = audioread('C:\Users\hp\Documents\Erasmus\Project\ASP_Project\chapters\ch-3\audio_samples\violin.wav');
    clf;
    specgram(input_signal,1024,Fs,256);
    auto_save_fig()
    soundsc(input_signal,Fs);
...
function Secure_Close()
    % In case of an error, turn to default setting on, so we can open the
    % saved files.
    fprintf('SECURITY ACTIVE: returning to default settings...\n');
    set(0, 'DefaultFigureVisible', 'on');
end

function auto_save_fig()
    persistent counter;
    if isempty(counter)
        counter = 1;
    else
    ...
    currentFolder = 'C:\Users\hp\Documents\Erasmus\Project\ASP_Project\chapters\ch-3\original-matlab';
    pngFolder = fullfile(currentFolder, '..','figures', 'MATLAB', 'png');
    figFolder = fullfile(currentFolder, '..','figures', 'MATLAB', 'fig');
    
    if ~exist(pngFolder, 'dir'), mkdir(pngFolder); end
    if ~exist(figFolder, 'dir'), mkdir(figFolder); end
    
    % sprintf: put 0's to empty places, the number are at least 2 digit,
    % numbers are decimals.
    fileName = sprintf('fig_%02d', counter);

    drawnow; % complete the drawing
    % set(gcf, 'ToolBar', 'none'); % hide tool bar
    % set(gcf, 'MenuBar', 'none'); % hide top menu
    % set(gcf, 'PaperPositionMode', 'auto');
    % set(gca, 'LooseInset', get(gca, 'TightInset') *1.5);

    set(gcf, 'Visible', 'on');
    savefig(gcf, fullfile(figFolder, [fileName, '.fig']));
    exportgraphics(gcf, fullfile(pngFolder, [fileName, '.png']),'BackgroundColor', 'white');
    % saveas(gcf, fullfile(pngFolder, [fileName, '.png']));
    % print(gcf, fullfile(pngFolder, [fileName, '.png']));
    set(gcf, 'Visible', 'off');
    disp([fileName,' saved to: ', pngFolder]);
    close(gcf);
end